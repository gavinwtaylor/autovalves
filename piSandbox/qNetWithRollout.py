...
def trainNet(trainname,numNodes,numLayers,epochs,lr,batchSize,numLoaders=1,\
  ...
  class ValveDataset(Dataset):
    def __init__(self, file, gamma, transform=None):
      f=h5py.File(file,'r')
    ...
    def __getitem__(self,idx):
      ...
      laststate=self.groupStates[groupIdx][-1,:]
      lastaction=self.groupActions[groupIdx][-1,:]
      rewardComponent=0
      for i in range(len(reward)):
        rewardComponent=rewardComponent+np.pow(self.gamma,i)*reward[i]
      gamPow=len(reward)
      sample={'state':state,\
              'action':action,\
  ...
  class ToTensor:
    def __call__(self,sample):
      s,a,r,sp,ap,gP=sample['state'],sample['action'],sample['reward'],\
                sample['laststate'],sample['lastaction'],sample['gamPow']
      s[0]=3*(s[0]-.75)
      s[1]=(s[1]-389)/21
      a[0]=3*(a[0]-1)
      ...
      return {'state':torch.from_numpy(s),\
              ...
              'reward':r,\
              'laststate':torch.from_numpy(sp),\
              'lastaction':torch.from_numpy(ap),\
              'gamPow':gP}
  ...
  for epoch in range(epochs):
    ...
    for i_batch,sample_batched in enumerate(dataloader):
      sps=Variable(sample_batched['laststate'],requires_grad=False).type(dtype)
      aps=Variable(sample_batched['lastaction'],requires_grad=False).type(dtype)
      rs=Variable(sample_batched['reward'],requires_grad=False).type(dtype)
      N=rs.shape[0]
      rs=rs.view(N,1)
      primes=torch.cat([sps,aps],dim=1)
      expectedNextvals=rs+np.pow(gamma,sample_batched['gamPow']*model(primes)
