...
def calcReward(states, x0scaleinv, x1scaleinv):
    n=states.shape[0]
    rewards=np.empty(n)
    for i in range(n):
        r = -(
              ((states[i][0] - states[i][2]) * x0scaleinv) * 
              ((states[i][0] - states[i][2]) * x0scaleinv) * 
              ((states[i][1] - states[i][3]) * x1scaleinv) * 
              ((states[i][1] - states[i][3]) * x1scaleinv)
             )
        rewards[i] = r
    return rewards
...
if __name__ == '__main__':
  ...
  for name in loglist: #retrieving data from each of the log files
    ...
    rewards = calcReward(obs, x0scaleinv, x1scaleinv)
    numruns = 0
    lastone=-1
    thisone=0
    ...
    for ind, j in enumerate(masks):
      if j == True:
        ...
        dset1=grp.create_dataset("states",(thisone-lastone,4), dtype=obs.dtype)
